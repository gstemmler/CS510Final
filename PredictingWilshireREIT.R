...
pdf("reg_monthly_a residuals graph.pdf")
...
pdf("lmMod_bc graphs.pdf")
plot(lmMod_bc)
dev.off()

## ggplot of residuals for reg1b
data$model <- stats::predict(reg1b, newdata=data)
err <- stats::predict(reg1b, newdata=data, se = TRUE)
data$ucl <- err$fit + 1.96 * err$se.fit
data$lcl <- err$fit - 1.96 * err$se.fit

pdf("ggplot of reg1b residuals graph.pdf")
ggplot(data) + geom_point(aes(x=Y_DiffLog, y = model), size = 2, colour = "blue") + geom_smooth(data=data, aes(x=Y_DiffLog, y=model, ymin=lcl, ymax=ucl), size = 1.5, colour = "red", se = TRUE, stat = "smooth")
dev.off()
